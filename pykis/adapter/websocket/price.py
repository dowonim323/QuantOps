...
class KisWebsocketQuotableProduct(Protocol):
    ...
    def on(
        ...
        Args:
            event (Literal["market_status"]): 이벤트 타입
            ...
            once (bool, optional): 한번만 실행할지 여부. Defaults to False.
            extended (bool, optional): (미사용) 호환성 유지를 위한 인자
        """
        ...

    def on(
        self,
        event: Literal["price", "orderbook", "market_status"],
        callback: (
            ...
            | Callable[[KisWebsocketClient, KisSubscriptionEventArgs[KisRealtimeMarketStatus]], None]
        ),
        where: (
            ...
            | KisEventFilter[KisWebsocketClient, KisSubscriptionEventArgs[KisRealtimeMarketStatus]]
            | None
        ) = None,
        once: bool = False,
        extended: bool = False,
...
class KisWebsocketQuotableProductMixin:
    ...
    def on(
        ...
        elif event == "market_status":
            from pykis.api.websocket.market_status import (
                on_product_market_status as on_market_status,
            )
